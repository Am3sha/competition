...
app.post('/api/submit', upload.array('screenshots', 3), async (req, res) => {
  try {
    const {
      name,
      phone,
    ...
    // Debug log: what files were received by multer
    console.log('RECV files count=', files.length, 'names=', files.map(f => f.originalname));
    ...
    // Debug log: what public URLs were produced
    console.log('Produced screenshotUrls:', screenshotUrls);
